...
def update(request, pk):
    ...
    if request.method == "POST":
        ...
        if check_password(form_master_password, user.password):
            encrypted_password = encrypt(form_master_password.encode(), website_password.encode())
            location.website_password = encrypt(form_master_password.encode(), encrypted_password)
            location.save()
            messages.success(request, "Password updated successfully.")
            return redirect("view", pk=location.id)
        else:
